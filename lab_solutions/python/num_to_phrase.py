...
# if number is greater than 100 add xhundred to string
if user_input >= 100:
    ten_digit = user_input // 10 % 10
    output += f"{single_digits[hundred_digit]} hundred "

# if last two digits are greater than 20 add tens to output string
if user_input % 100 >= 20:
    output += f"{double_digits[ten_digit]} "

    if user_input % 10 < 10:
        # add ones to output string
        output += f"{single_digits[single_digit]}"

# if last two digits are between 10-19 add teens to output string
elif user_input % 100 > 9:
    output += f"{tens_digits[single_digit]}"
